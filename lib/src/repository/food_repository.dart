...
Future<Food> nitingetFood(String foodId) async {
  Uri uri = Helper.getUri('api/foods/$foodId');
  uri = uri.replace(queryParameters: {'with': 'nutrition;restaurant;category;extras;extraGroups;foodReviews;foodReviews.user'});
  try {
    final client = new http.Client();
    ...
    final foodResponse =  decodedResponse["data"];
    Food food1;
    String foodJson = jsonEncode(foodResponse);
    food1 = Food.fromJSON(foodResponse);
    return food1;
    /*
//    String rawJson = jsonEncode(decodedResponse);
 //   food1 = Food.fromJSON(foodResponse);
   // String successmessage = rawJson.success;
    final streamedRest = await client.send(http.Request('get', uri));
